...
img_list_path <- 'pikachu/data/train_img_list.RData'
box_info_path <- 'pikachu/data/train_box_info.RData'

# Read annotations

train_box_info <- read.table(train_annot_path, header = TRUE, stringsAsFactors = FALSE)
save(train_box_info, file = box_info_path)

# Start to preprocessing (image)

train_img_list <- list()

train_img_id <- unique(train_box_info$img_id)
num_img <- length(train_img_id)

t0 <- Sys.time()

for (i in 1:num_img) {
  
  img <- try(readJPEG(paste0(train_data_dict, train_img_id[i], '.jpeg')), silent = TRUE)
  
  if (is.array(img)) {
    
    train_img_list[[i]] <- writeJPEG(img, quality = 1)
    
    if (i %% 100 == 0) {
      Show_img(img = readJPEG(train_img_list[[i]]),
               box_info = train_box_info[train_box_info[,'img_id'] == train_img_id[i],])
...
save(train_img_list, file = img_list_path)
